...
# Parse WEAVIATE_URL if set (e.g. http://weaviate:8080), else fall back to
# individual WEAVIATE_HOST / WEAVIATE_PORT env vars.
_weaviate_url = os.getenv("WEAVIATE_URL")
if _weaviate_url:
    _parsed = urlparse(_weaviate_url)
    WEAVIATE_HOST = _parsed.hostname or "localhost"
    WEAVIATE_PORT = _parsed.port or 8080
else:
